...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto flex justify-center h-16 gap-4 items-end rounded-2xl bg-transparent px-4 pb-3",
        className
      )}
    >
      {items.map((item) => (
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className={cn(
                "px-2 py-1 whitespace-pre rounded-md border absolute left-1/2 -translate-x-1/2 -top-12 w-fit text-sm",
                hoverTitleClassName
              )}
            >
              {title}
